...
async function computeDecisions(shopId, query = {}) {
  {
    ...
    const [salesRes, invRes, purchRes, bakayaInvRes, expRes, onlineRes, cashRes] = await Promise.allSettled([
      ...
      supabase.from("invoices")
        ...
        .eq("shop_id", shopId).in("payment_status", ["credit", "partial"]).limit(2000),
      supabase.from("expenses").select("amount, expense_date, category").eq("shop_id", shopId).gte("expense_date", since),
    ...
    return {
      ...
      // What the shop must sell each month before it earns anything. Absent unless the
      // expenses actually carry a fixed category — reporting zero fixed cost would say
      // "you break even immediately", which is a confident lie.
      breakEven: (() => {
        const fixed = advice.monthlyFixedCosts(expenses, { days: windowDays });
        if (!fixed) return null;
        const revenue = Object.values(perDesign).reduce((t, x) => t + x.revenue, 0);
        const cogs = Object.entries(perDesign)
          .reduce((t, [d, x]) => t + x.units * (lastCost[d] || 0), 0);
        const be = advice.shopBreakEven({
          revenue, cogs, fixedPerMonth: fixed.perMonth, windowDays,
        });
        if (!be) return null;
        return {
          ...be,
          assumption: "Fixed = rent, bijli-paani aur salary. Transport aur marketing bikri ke saath badalte hain, isliye fixed me nahi gine.",
        };
      })(),
